...
$('#form-tambah-produk').submit(function(e){
    e.preventDefault();
    if ($('#tambah-gambar-input').val().length == 0) {
        $('#tambah-gambar-preview').addClass('invalid');
        $('#invalid-gambar').show();
...
$('#tambah-gambar-input').on('input', function(){
    $('#tambah-gambar-preview').removeClass('invalid');
    $('#invalid-gambar').hide();
})
...
var update_gambar_input = document.querySelector('#update-gambar-input');
update_gambar_input.addEventListener('change', update_preview);
function update_preview(){
    ...
    fileReader.onload = function(){
        var result = fileReader.result;
        var img = document.querySelector('#update-gambar-preview');
        img.setAttribute('src', result);
    }
}

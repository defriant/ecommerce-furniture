    ...
    <div class="container">
        ...
        @if (Auth::user()->pesanan->count() > 0)
            @foreach (Auth::user()->pesanan->sortByDesc('created_at') as $p)
                ...
                @elseif($p->jenis_pesanan == 'custom_pesanan')
                    <div class="pesanan-list">
                        <div class="panel panel-default">
                            <div class="panel-head">
                                <h5 style="font-size: 12px"><i class="fas fa-shopping-bag" style="margin-right: 5px; color: coral"></i>Custom Pesanan</h5>
                                <h5 style="font-size: 12px; margin-left: 10px">{{ date("d M Y", strtotime($p->created_at)) }}</h5>
                                @if ($p->status == 'menunggu_konfirmasi')
                                    <h5 class="status-process">Menunggu Konfirmasi Pesanan</h5>
                                @elseif ($p->status == 'konfirmasi')
                                ...
                                @elseif ($p->status == 'menunggu_validasi')
                                    <h5 class="status-process">Menunggu Validasi Pembayaran</h5>
                                @elseif ($p->status == 'validasi')
                                    <h5 class="status-process">Menunggu Stok Barang</h5>
                                @elseif ($p->status == 'validasi_invalid')
                                    <h5 class="status-invalid">Pembayaran Tidak Valid</h5>
                            ...
                            <div class="panel-body">
                                <div class="col-xs-12 col-md-8">
                                    ...
                                    <div class="col-xs-12 col-sm-6" style="text-align: center">
                                        ...
                                        @elseif($p->status == 'konfirmasi' || $p->status == 'menunggu_validasi' || $p->status == 'validasi')
                                        <h5 style="font-size: 13px; margin-bottom: 10px">Estimasi Pengerjaan :</h5>
                                        <h5 style="font-size: 13px">{{ date('d M Y', strtotime($p->estimasi_stok)) }}</h5>
                                        @elseif ($p->status == 'stok_ready' || $p->status == 'pengiriman' || $p->status == 'tiba_di_tujuan')
